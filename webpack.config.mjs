...
import WatchFilePluginImport from '@mytonwallet/webpack-watch-file-plugin';
...
// Дублируем значение из src/config.ts, чтобы webpack-config не импортировал TS при запуске Node.
...
const WatchFilePlugin = WatchFilePluginImport?.default ?? WatchFilePluginImport;
const StatoscopeWebpackPlugin =
  StatoscopeWebpackPluginImport?.default ?? StatoscopeWebpackPluginImport;
...
// GitHub workflow uses an empty string as the default value if it's not in repository variables,
// so we cannot define a default value here
...
export default function createConfig(_, { mode = 'production' } = {}) {
  let server = 'http';
  ...
  return {
    ...
    module: {
      rules: [
        ...
        },
        {
          ...
          use: [
            MiniCssExtractPlugin.loader,
            {
              ...
              options: {
                importLoaders: 1,
                modules: { namedExport: false, auto: true },
              },
            },
            'postcss-loader',
          ],
        ...
        { test: /\.(txt|tl|strings)$/i, type: 'asset/source' },
      ],
    },
    ...
    plugins: [
      ...
      new HtmlWebpackPlugin({
        appTitle: APP_TITLE,
        appleIcon:
          APP_ENV === 'production' ? 'apple-touch-icon' : 'apple-touch-icon-dev',
      ...
      new EnvironmentPlugin({
        ...
        APP_NAME: null,
        APP_TITLE,
        // В прод-сборке эти значения могут отсутствовать локально — не валим сборку.
        TELEGRAM_API_ID: '',
        TELEGRAM_API_HASH: '',
        // eslint-disable-next-line no-null/no-null
        TEST_SESSION: null,
      ...
      new DefinePlugin({
        ...
        CHANGELOG_DATETIME: DefinePlugin.runtimeValue(() => {
          return JSON.stringify(
            statSync(CHANGELOG_PATH, { throwIfNoEntry: false })?.mtime.getTime(),
          );
        }, {
      ...
      new StatoscopeWebpackPlugin({
        statsOptions: { context: __dirname },
      ...
      new WatchFilePlugin({
        rules: [
          { files: 'src/assets/localization/fallback.strings', action: 'npm run lang:ts' },
          {
    ...
    optimization: {
      ...
      ...(APP_ENV === 'staging' && { chunkIds: 'named' }),
    },
  };
}
